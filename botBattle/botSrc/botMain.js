...
async function main() {
  const room1Data = await (await fetch("./botBattle/botMaps/room1.json")).json();
  const room2Data = await (await fetch("./botBattle/botMaps/room2.json")).json();
...
k.scene("intro", () => {
  ...
  k.add(
    makeNotificationBox(
      k,
      "Escape the factory!\nUse arrow keys to move, x to jump, z to attack.\n\nPress Enter to start!\n\n(For best experience, play on desktop with sound on)"
    )
  );
  k.onKeyPress("enter", () => {
